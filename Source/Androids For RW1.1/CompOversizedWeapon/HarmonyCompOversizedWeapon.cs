...
namespace MOARANDROIDS
{
    ...
    internal static class HarmonyCompOversizedWeapon
    {
        ...
        {
            if (!(eq is ThingWithComps thingWithComps)) return true;
            //If the deflector is active, it's already using this code.
            var deflector = thingWithComps.AllComps.FirstOrDefault(y =>
            {
                var memberInfo = y.GetType().BaseType;
                return memberInfo != null && (y.GetType().ToString() == "CompDeflector.CompDeflector" ||
                                              memberInfo.ToString() == "CompDeflector.CompDeflector");
            });
            if (deflector != null)
            {
            ...
            var matSingle = eq.Graphic is Graphic_StackCount graphic_StackCount ? graphic_StackCount.SubGraphicForStackCount(1, eq.def).MatSingle : eq.Graphic.MatSingle;
            ...
            if (compOversizedWeapon.Props == null || !compOversizedWeapon.Props.isDualWeapon) return false;
        ...
        private static float AdjustNonCombatRotation(Thing pawn, float num, CompOversizedWeapon compOversizedWeapon)
        {
            if (compOversizedWeapon.Props == null) return num;
        ...
        private static Vector3 AdjustRenderOffsetFromDir(Pawn pawn, CompOversizedWeapon compOversizedWeapon)
        {
            ...
            if (compOversizedWeapon.Props == null) return curOffset;
        ...
        public static void get_Graphic_PostFix(Thing __instance, ref Graphic __result)
        {
            ...
            if (tempGraphic == null) return;

            if (!(__instance is ThingWithComps thingWithComps)) return;
            ...
            var compOversizedWeapon = thingWithComps.TryGetComp<CompOversizedWeapon>();
            if (compOversizedWeapon == null) return;
